...
<a href="{{ route('Notification.Read') }}" class="float-right text-light">
    Mark all as read
</a>
</div>
</li>
...
<li class="notification-box">
    <div class="row">
        ...
        <strong class="text-info">{{ $notifications->data['user_create'] }}</strong>
        <div>
            <a href="{{ route('posts.show',$notifications->data['post_id']) }}">
                {{ $notifications->data['title_post'] }}
            </a>
    
    </div>
    <small class="text-warning">{{ $notifications->created_at}}</small>
    </div>
    </div>
</li>
@endforeach
